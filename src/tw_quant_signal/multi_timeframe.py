...
def _signal_type(daily_int: int, weekly_int: int) -> str:
    ...
    - Short-term (1-5 days): daily has signal but weekly doesn't agree
    - Swing (1-4 weeks): weekly has strong signal, daily confirming
    - Both: both aligned
    """
    if daily_int >= 4 and weekly_int >= 4:
        return "both"
    if weekly_int >= 4:
        return "swing"
    if daily_int >= 4:
        return "short"
    if daily_int <= 2 and weekly_int <= 2:
        return "both"
    if weekly_int <= 2:
        return "swing"
    if daily_int <= 2:
...
def compute_multi_timeframe(db: SignalDB, trade_date: Optional[str] = None) -> list[dict]:
